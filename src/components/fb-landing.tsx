...
import { FB_VARIANTS, type FbVariant } from "@/lib/fb-variants";
...
interface FbLandingProps {
  variant: FbVariant;
  stats: Stats;
  source: string; // tracked into HeroSignupForm for funnel attribution
}

export function FbLanding({ variant, stats, source }: FbLandingProps) {
  ...
  return (
    <main className="min-h-screen bg-[var(--navy)]">
      ...
      <section className="px-5 py-10 bg-white/[0.02] border-y border-white/10">
        <div className="max-w-xl mx-auto grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <div className="text-xs text-[var(--gray-dim)] mt-1 uppercase tracking-wider">
              Claims Processed
            </div>
          </div>
          <div>
            ...
            {[
              {
                ...
                body:
                  "AI annotates every photo, identifies damage type, cites building codes, and writes the narrative. PDF ready to hand the homeowner or adjuster.",
              ...
              },
              {
                ...
                body:
                  "Full line-item estimate with current market pricing — roofing, underlayments, flashings, gutters, siding. O&P auto-applied when 3+ trades.",
              ...
              },
              {
                ...
                body:
                  "AI finds every missing line, shorted quantity, and code violation the carrier ignored — then composes the supplement email and attaches the evidence package.",
                accent: "from-[var(--blue)] to-[var(--cyan)]",
              },
            ].map((s) => (
              <div
                key={s.step}
                className="relative rounded-2xl bg-white/[0.03] border border-white/10 p-5 sm:p-6"
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`shrink-0 w-9 h-9 rounded-full bg-gradient-to-br ${s.accent} flex items-center justify-center text-white font-bold`}
                  >
                    {s.step}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs uppercase tracking-wider text-[var(--gray-muted)] mb-0.5">
                      You add
                    </div>
                ...
                <div className="pl-12">
                  <div className="text-xs uppercase tracking-wider text-[var(--gray-muted)] mb-1">
                    AI delivers
                  </div>
      ...
      <section className="px-5 py-12 bg-gradient-to-b from-[var(--navy)] to-[var(--navy-light)]">
        <div className="max-w-xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3">{cfg.finalHeadline}</h2>
          <p className="text-[var(--gray-dim)] mb-6">{cfg.finalSubcopy}</p>
          <HeroSignupForm source={`${source}_bottom`} />
        </div>
      </section>
