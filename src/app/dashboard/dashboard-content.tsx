...
export function DashboardContent({ user }: { user: User }) {
  ...
  const [winBannerCollapsed, setWinBannerCollapsed] = useState<boolean>(() => {
    if (typeof window === "undefined") return true;
    return window.localStorage.getItem("dashboard.winBannerCollapsed") !== "false";
  });
                ...
                <div className="relative p-8">
                  <div className="flex items-center justify-between flex-wrap gap-6">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const next = !winBannerCollapsed;
                        setWinBannerCollapsed(next);
                        if (typeof window !== "undefined") {
                          window.localStorage.setItem("dashboard.winBannerCollapsed", String(next));
                        }
                      }}
                      aria-expanded={!winBannerCollapsed}
                      aria-label={winBannerCollapsed ? "Show winning claims" : "Hide winning claims"}
                      className="self-start ml-auto flex items-center gap-2 bg-white/15 hover:bg-white/25 backdrop-blur rounded-full px-4 py-2 text-xs font-semibold text-white transition-colors"
                    >
                      {winBannerCollapsed ? "Show wins" : "Hide wins"}
                      <svg
                        className={`w-4 h-4 transition-transform ${winBannerCollapsed ? "" : "rotate-180"}`}
